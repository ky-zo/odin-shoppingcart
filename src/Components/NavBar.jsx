    ...
    return (
        <header className="grid grid-cols-3 justify-between bg-white bg-opacity-30 backdrop-blur-md py-2 px-4 sticky top-0 z-10 shadow-lg">
            <NavLink to="/" className="flex items-center gap-2">
                ...
                <div className="text-lg font-bold hidden sm:flex">Foodey</div>
            </NavLink>
            <nav className="flex justify-center items-center gap-2 sm:gap-8">
                {menuElements.map((element) => (
                    <div
                        key={element}
                        className="font-semibold text-base sm:text-lg hover:text-purple-500 transition-all duration-300 hover:text-xl"
                    >
